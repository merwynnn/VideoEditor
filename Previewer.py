...
class Previewer:
    ...
    def frame(self, events, mouse_pos):
        if not self._needs_update and not self.videoEditor.is_playing and not self.has_cursor_moved and not self.videoEditor.reload_all and not self.videoEditor.file_browser.is_dragging_video():
